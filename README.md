...
- `ipts_set_snat6`：是否设置 IPv6 的 MASQUERADE 规则，默认为 true，通常情况下你必须设置为 true（除非你使用 GUA 地址进行透明代理，但是这样会让配置变得复杂），因为 IPv6 的透明代理需要理由 ULA 内网地址，如果设置为 false，那么其它内网主机将无法再访问 IPv6 网络，因为 ULA 地址是不可以在公网上被路由的，因此你必须进行 SNAT 源地址转换。
