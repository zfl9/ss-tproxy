...
在内网主机 192.168.1.100 上运行端口为 8443 的服务进程，然后我们从其它外网主机（假设 IP 为 2.2.2.2）上连接此端口上的服务，会发生什么呢？首先外网主机向拨号网关的 8443 端口发起连接（假设公网 IP 为 1.1.1.1），简单表示为 `2.2.2.2:2333 -> 1.1.1.1:8443`，然后拨号网关查询到对应的端口映射规则，于是做 DNAT 转换，变为 `2.2.2.2:2333 -> 192.168.1.100:8443`，然后通过内网网卡送到了 192.168.1.100 主机的 8443 端口（SYN 握手请求成功到达）；然后服务进程将发送 SYN+ACK 握手响应包，即 `192.168.1.100:8443 -> 2.2.2.2:2333`，因为内网主机的网关设为了 192.168.1.2，所以 SYN+ACK 包将被送到代理网关上，因为目的地址 2.2.2.2 并没有在 gfwlist 列表中，所以放行，经过 FORWARD 链，到达 POSTROUTING 链，问题来了，ss-tproxy 已经在 POSTROUTING 链的 nat 表上设置了 SNAT 规则（`ipts_non_snat` 为 false），所以该数据包将会被转换为 `192.168.1.2:6666 -> 2.2.2.2:2333`，而当这个数据包到达拨号网关时，拨号网关检查发现这个源地址并不是 192.168.1.100:8443，所以不会按照端口映射规则将数据包转换为 `1.1.1.1:8443 -> 2.2.2.2:2333`，而是将其映射为一个随机端口，比如 62333，所以外网主机接收到的 SYN+ACK 包的源地址是 1.1.1.1:62333，显然无法握手成功，导致端口映射失败。
