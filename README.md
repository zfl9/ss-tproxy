...
- `opts_hostname_resolver`：告诉 ss-tproxy，使用哪个工具来解析 `proxy_svraddr4/6` 中的域名；默认为 `auto`，auto 模式的查找优先级为 `dig`、`getent`、`ping`，只要找到其中一个，就不会继续搜寻下去了；dig 需要自己安装 bind-utils/dnsutils 包，getent 大多数发行版都自带了，ping 基本上是个系统都有；如果你的系统只有 ping 命令，且能够明显感受到解析延迟，那么请安装 dig 实用工具（Debian 系列基本都有这个问题，busybox 版本的 ping 也一样）。
