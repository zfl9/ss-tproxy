...
- `proxy_tcpport/proxy_udpport`：本机代理进程的透明代理监听端口，前者为 TCP 监听端口，后者为 UDP 监听端口，通常情况下 TCP 端口与 UDP 端口是相同的，根据实际情况进行修改。需要注意的是，ss-tproxy v3.0 之后都要求代理软件支持 UDP，否则 DNS 是无法正常解析的，请务必检查 UDP 代理的连通情况，对于 ss/ssr，它们的 UDP 代理是使用 UDP 协议访问 VPS 的，某些 ISP 可能会对 UDP 进行恶意丢包，v2ray 某些代理协议则为 UDP over TCP，对于这种情况则无需担心 UDP 丢包问题。
