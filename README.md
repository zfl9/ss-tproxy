...
**`proxy_svraddr4`**：填写 VPS 服务器的外网 IPv4 地址，IP 或域名都可以，填域名需要注意一点，这个域名最好不要有多个 IPv4 地址与之对应，因为脚本内部只会获取其中某个 IP，这极有可能与本机代理进程解析出来的 IP 不一致，不一致导致的结果就是 iptables 规则死循环，所以应尽量避免这种情况，比如将域名与 IP 的映射关系写到 ss-tproxy 主机的 `/etc/hosts` 文件中。可以填写多个 VPS 地址，用空格隔开，填写多个地址的作用是为了方便切换代理，比如我现在有两个 VPS，VPS-A、VPS-B，假设你先使用 VPS-A，因为某些不可抗拒因素，导致 VPS-A 的网络性能低下，那么你可能就要切换到 VPS-B，如果你只填写了 VPS-A 的地址，那么你就需要去修改 ss-tproxy.conf，将地址改为 VPS-B，还得修改 `proxy_startcmd`，最后还得重启 ss-tproxy 脚本，非常麻烦，而且更麻烦的是，如果现在 VPS-A 的网络又好了，那么你可能又想切换回 VPS-A，那么你又得重复上述步骤，改配置，重启脚本。但现在，你不需要这么做，你完全可以在 `proxy_svraddr4` 中填写 VPS-A 和 VPS-B 的地址，假设你默认使用 VPS-A（`proxy_startcmd`），那么启动 ss-tproxy 后，默认使用的就是 VPS-A，此后如果你想切换为 VPS-B，仅需要 stop VPS-A 的本机代理进程，然后 start VPS-B 的本机代理进程，切换回 VPS-A 的步骤也是差不多的，该过程无需操作 ss-tproxy。
