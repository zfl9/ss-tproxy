...
- REDIRECT：REDIRECT 其实就是 DNAT，所谓 DNAT 就是直接修改数据包的目的地址及目的端口，在 REDIRECT 中，目的地址会被改为环回地址（`127.0.0.1`、`::1`），目的端口会被改为我们所指定的端口号；以 IPv4 下的 TCP 协议为例，假设我们给 REDIRECT 指定的端口号为 60080，当 netfilter 发现目的地址及目的端口为 `1.2.3.4:80` 的数据包匹配该规则时，会先将这个数据包的目的地址和目的端口暂时存起来，然后修改数据包的目的地址和目的端口为 `127.0.0.1:60080`；当 client 进程收到该数据包后，就会调用 netfilter 的 API 来获取这个数据包的原始目的地址及原始目的端口，也就是 `1.2.3.4:80`，拿到目的地址和端口之后就可以进行相关的代理操作了，比如通过私有协议发送给境外服务器上的 server 进程，然后由 server 进程“替代”client 进程去访问这个目的地址。
- TPROXY：// TODO
