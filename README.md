...
但在实际测试中发现一个棘手的问题，我们的设备具有两个 IPv6 地址，当设备准备发送一个 IP 包时，会遇到源地址选择问题，比如它可能会选择 GUA 地址作为该 IP 包的源 IP，也可能会选择 ULA 地址作为该 IP 包的源 IP，如果它选择 ULA 地址作为源 IP，那没什么问题，符合我们的预期，但如果它选择 GUA 地址作为源 IP，就有问题了，这种情况将导致 ss-tproxy 主机的 iptables 规则无法拿到这些数据包，因为它们的源 IP 不再是 ULA 私有地址，而是 GUA 公网地址，这些数据包实际上会被直接转发到国内网络，并没有走代理。
