...
- `proxy_svrport`：填写 VPS 上代理服务器的外部监听端口，格式同 `ipts_proxy_dst_port`，不以留空，实际端口是多少就填多少，填写不正确会导致 iptables 规则死循环。如果是 v2ray 动态端口范围，比如端口号 1000 到 2000 都是代理监听端口，那么就可以填 `1000:2000`（含边界）。
