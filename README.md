...
ss-tproxy 主机也有一个 GUA 地址，一个 ULA 地址，它的默认网关应该保持不变，即还是原有的 GUA 默认网关，这样才能保证 ss-tproxy 主机能够正常访问 IPv6；而其它主机的默认网关则应该修改为 ss-tproxy 主机的 ULA 地址，而不再是 DHCP 分配的 GUA 网关，这样它们发出的流量才会经过 ss-tproxy。然后我们在 `ipts_intranet6` 中填写此 ULA 网段，同时启用 `ipts_set_snat6` 选项，做源地址转换，因为 ULA 地址需要转换为 GUA 地址才能正常路由。这其实和我们现在的 IPv4 NAT 网络差不多，不同的是，这些设备具有一个 IPv6 公网地址，因此我们仍然可以在外部 IPv6 网络中访问这些具有 GUA 地址的设备。
