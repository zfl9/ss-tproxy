...
- `opts_ip_for_check_net`：指定一个允许 Ping 的 IP 地址（IPv4 或 IPv6 都行），用于检查外部网络的连通情况，默认为 `114.114.114.114`，注意这个 IP 地址应该为公网 IP，如果你填一个私有 IP，那么即使检测成功，也不能保证外网是可访问的，因为这仅仅代表我可以访问这个内网。可根据实际网络环境进行更改，一般改为当前网络中延迟较低且较稳定的一个 IP。
