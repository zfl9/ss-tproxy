...
ss-tproxy v4.0 版本需要利用 ULA 地址进行 IPv6 透明代理，而且还有许多要注意的事项，体验不是很好；但 v4.5 版本不需要任何额外的配置，如果想使用 IPv6 透明代理，直接启用 `ipv6` 选项即可，使用方法完全同 IPv4 透明代理。当然，v4.5 版本依旧可以使用 ULA 地址来进行透明代理（如忍受不了 GUA 地址总是变化），使用 ULA 地址做透明代理仅需注意一点：将 ss-tproxy.conf 配置文件中的 `ipts_set_snat6` 选项设为 true。
